...
export default function Projects() {
  ...
  const filteredProjects = portfolioData.projectsSection.projects.filter((project) => {
    if (activeCategory === "All") return true;
    return project.category === activeCategory;
  });

  return (
    <section id="projects" className="py-24 bg-slate-50/60 border-t border-slate-200/80 relative overflow-hidden">
          ...
          <AnimatePresence mode="wait">
            {filteredProjects.map((project, idx) => (
              <motion.div
                ...
                className="glass-card rounded-3xl p-6 sm:p-10 border border-slate-200 overflow-hidden"
              >
                  ...
                  <div className="lg:col-span-5 flex justify-center">
                    <div className="w-full bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-2xl text-white shadow-xl flex flex-col justify-between min-h-[280px]">
                      <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-4">
                        ...
                        <span className="text-[11px] font-mono text-slate-400">production_build_v1.0</span>
                      </div>

                      <div className="space-y-4 my-auto">
                        <div className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/10">
                          <span className="text-xs font-semibold text-slate-300">Target Environment</span>
                          <span className="text-xs font-mono font-bold text-blue-400">Enterprise Web / Mobile</span>
                        </div>

                        <div className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/10">
                          <span className="text-xs font-semibold text-slate-300">Measured Impact</span>
                          <span className="text-xs font-mono font-bold text-emerald-400">{project.metrics}</span>
                        </div>
                      </div>

                      <div className="pt-4 border-t border-white/10 text-right">
                        <span className="text-[11px] text-slate-400">Verified by Frontend Audit</span>
                      </div>
                    </div>
                  </div>
